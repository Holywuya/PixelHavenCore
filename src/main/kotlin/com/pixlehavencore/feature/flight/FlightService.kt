...
object FlightService {
    ...
    private val playerData = ConcurrentHashMap<UUID, FlightPlayerData>()
                ...
                if (player.isFlying) {
                    ...
                    if (data.effectiveSeconds <= 0) {
                        ...
                        if (FlightSettings.msgTimeExpired.isNotBlank()) {
                            player.sendActionBar(TextUtils.parse(FlightSettings.msgTimeExpired))
                        }
                        continue
                    }
                    val newData = decrementTime(data)
                    playerData[uuid] = newData
                    sendActionBar(player, newData)
                }
            }
        }
    }
    ...
    private fun sendActionBar(player: Player, data: FlightPlayerData) {
        ...
        player.sendActionBar(TextUtils.parse(message))
    }
