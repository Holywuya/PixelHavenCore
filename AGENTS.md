...
- `src/main/resources/menus/**`
  - GUI 菜单模板与示例配置，供 `feature/menu` 与 `feature/menu/shop` 参考。
...
- `feature/tax`
  - 交易税池、税率计算、税收资金管理。
- `feature/economy`
  - 内置经济模块，基于 VaultUnlockedAPI 提供多货币余额、存取与服务注册。
- `feature/fishing`
  - 自定义钓鱼奖励与掉落池。
- `feature/mobdrop`
  - 自定义怪物掉落与金钱掉落。
- `feature/mythicmobsextra`
  - MythicMobs 增强模块集合（如 MM 怪物发光）。
...
- 🔴 **动态 Section 白名单（ConfigAlignService）**：`ConfigAlignService` 会在启动时对齐配置文件键，但**用户可自由扩展的 section 必须加入白名单**，否则用户自定义的数据将被删除。
  ...
  - **当前白名单**（截至本版本）：
    ...
    | `feature/tax.yml` | `tax-brackets` | 阶梯税率档位，可自由增删 |
    | `feature/economy.yml` | `currencies` | 货币定义，可自由增删币种 |
