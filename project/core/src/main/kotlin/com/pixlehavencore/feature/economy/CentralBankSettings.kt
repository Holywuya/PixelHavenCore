...
object CentralBankSettings {
    ...
    fun reload() {
        ...
        expectedBalance = BigDecimal.valueOf(config.getDouble("expected-balance", 10000.0))
            .coerceAtLeast(BigDecimal.ONE)
        bufferMultiplier = BigDecimal.valueOf(config.getDouble("buffer-multiplier", 2.0))
            .coerceAtLeast(BigDecimal.ONE)
        ...
        dormantRecoveryRate = BigDecimal.valueOf(config.getDouble("dormant-recovery-rate", 0.01))
