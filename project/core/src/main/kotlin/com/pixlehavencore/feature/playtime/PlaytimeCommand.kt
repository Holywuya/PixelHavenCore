...
object PlaytimeCommand {
    ...
    val main = mainCommand {
        execute<ProxyCommandSender> { sender, _, _ ->
            sender.msgRaw("<gold>=== 在线时长命令帮助 ===")
            sender.msgRaw("<aqua>/playtime <gray>- 查看自己的在线时长")
            sender.msgRaw("<aqua>/playtime <玩家> <gray>- 查看他人在线时长")
            sender.msgRaw("<aqua>/playtime top [类型] [数量] <gray>- 查看排行榜")
            sender.msgRaw("<aqua>/playtime cleanup [天数] <gray>- 清理旧数据")
            sender.msgRaw("<aqua>/playtime reload <gray>- 重载配置")
            sender.msgRaw("<gray>类型: total(默认) / today / week / month")
        }
    }
        ...
        dynamic(comment = "days") {
            ...
            literal("confirm") {
                execute<ProxyCommandSender> { sender, context, _ ->
                    if (!sender.requirePermission(ADMIN_PERMISSION)) return@execute
                    val days = context.getOrNull("days")?.toString()?.toIntOrNull() ?: PlaytimeSettings.cleanupDefaultDays
                    sender.msgRaw("<gray>正在清理超过 $days 天未登录的玩家数据...")
    ...
    private fun handleLeaderboard(sender: ProxyCommandSender, type: String, limit: Int) {
        ...
        sender.msgRaw("<gold>=== 在线时长排行（$typeLabel）Top $limit ===")
        PlaytimeService.queryLeaderboard(type, limit) { entries ->
            submit {
                if (entries.isEmpty()) {
                    sender.msgRaw("<gray>暂无数据。")
                    return@submit
                }
                entries.forEach { entry ->
                    sender.msgRaw("<yellow>#${entry.rank} <white>${entry.playerName} <gray>- <aqua>${entry.playtimeFormatted}")
                }
            }
        }
    }
    ...
    private fun showCleanupPreview(sender: ProxyCommandSender, days: Int) {
        ...
        preview.take(10).forEach { (_, name) ->
            sender.msgRaw("<gray>- $name")
        }
        if (preview.size > 10) {
            sender.msgRaw("<gray>...及其他 ${preview.size - 10} 位")
        }
