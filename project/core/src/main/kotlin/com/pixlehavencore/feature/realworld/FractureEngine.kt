...
object FractureEngine {
    ...
    fun onFallDamage(player: Player, state: PlayerEnvState, event: EntityDamageEvent) {
        if (!RealWorldSettings.fractureEnabled) return
        if (event.cause != EntityDamageEvent.DamageCause.FALL) return

        val damage = event.finalDamage
        if (damage < RealWorldSettings.fractureMinFallDamage) return

        // 根据坠落伤害计算骨折值增量
        // 公式：(伤害 - 最小阈值) * 系数
        val fractureIncrease = (damage - RealWorldSettings.fractureMinFallDamage) * RealWorldSettings.fractureDamageMultiplier
    ...
    fun applyEffects(player: Player, state: PlayerEnvState, tickSeconds: Int) {
        ...
        if (state.fracture > 0) {
            // 基础恢复速率：每分钟 2 点
            var recoveryPerSecond = RealWorldSettings.fractureRecoveryRate / 60.0
    ...
    fun useTreatment(player: Player, state: PlayerEnvState, treatment: FractureTreatment): Boolean {
        if (!RealWorldSettings.fractureEnabled) return false
        if (state.fracture <= 0) {
        ...
        when (treatment) {
            FractureTreatment.BANDAGE -> {
                state.fracture = (state.fracture - RealWorldSettings.fractureBandageHealAmount).coerceAtLeast(0.0)
    ...
    fun classifyFracture(fractureValue: Double): FractureSeverity {
        return when {
            fractureValue < RealWorldSettings.fractureMildThreshold -> FractureSeverity.NONE
            fractureValue < RealWorldSettings.fractureModerateThreshold -> FractureSeverity.MILD
            fractureValue < RealWorldSettings.fractureSevereThreshold -> FractureSeverity.MODERATE
